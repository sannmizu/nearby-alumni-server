...
class Geohash
{
    ...
    public function expand($hash) {
        ...
        $downLat = self::calculate($bLat, -1);
        $upLat = self::calculate($bLat, 1);
        $leftLon = self::calculate($bLon, -1);
        $rightLon = self::calculate($bLon, 1);
        
        return array(
            "upleft" => self::base32_encode(self::merge_lat_lon($upLat, $leftLon)),
            "up" => self::base32_encode(self::merge_lat_lon($upLat, $bLon)),
            "upright" => self::base32_encode(self::merge_lat_lon($upLat, $rightLon)),
            "right" => self::base32_encode(self::merge_lat_lon($bLat, $rightLon)),
            "downright" => self::base32_encode(self::merge_lat_lon($downLat, $rightLon)),
            "down" => self::base32_encode(self::merge_lat_lon($downLat, $bLon)),
            "downleft" => self::base32_encode(self::merge_lat_lon($downLat, $leftLon)),
            "left" => self::base32_encode(self::merge_lat_lon($bLat, $leftLon))
        );
    }
    private function calculate($bstr, $i) {
        $length = strlen($bstr);
        $dstr = bindec($bstr);
        $result = decbin($dstr + $i);
    ...
    private function base32_decode($data) {
        ...
        for($i=0; $i<$hl; $i++)
        {
            $binary.=$this->codingMap[substr($data,$i,1)];
        }
        return $binary;
    }
    private function base32_encode($data) {
        ...
        {
            $n=bindec(substr($data,$i,5));
            $hash=$hash.$this->coding[$n];
        }
        return $hash;
    }
    private function merge_lat_lon($blat, $blong) {
        //merge lat and long together
        $binary="";
        $uselong=1;
        while (strlen($blat)+strlen($blong))
